...
export default function Home() {
  ...
  const idfRows = [...data.vocabulary].sort((a, b) => data.df[b] - data.df[a] || a.localeCompare(b, "ko")).slice(0, 15);
      ...
      <section className="lab" id="lab">
        ...
        <div className="workbench">
          ...
          <div className={`stage ${motionEnabled ? "motion-on" : "motion-off"}`}>
            ...
            {(pdfMode || step === 7) && <div className="panel"><PanelTitle n="07" title="TF-IDF 값이 큰 순서로 정리해요" text="각 단어가 댓글을 대표하는 정도를 비교할 수 있도록 TF-IDF가 큰 순서로 최대 15개를 정리했어요." /><table><thead><tr><th>순위</th><th>단어</th><th>가장 큰 TF-IDF</th><th>댓글별 값</th></tr></thead><tbody>{data.tfidfRows.slice(0, 15).map((item, index) => <tr key={item.word}><td>{index + 1}</td><td><b>{item.word}</b></td><td>{item.score.toFixed(2)}</td><td>{item.values.map((value, i) => `댓글 ${i + 1}: ${value.toFixed(2)}`).join(" · ")}</td></tr>)}</tbody></table></div>}
            ...
            {(pdfMode || step === 9) && <div className="panel"><PanelTitle n="09" title="코사인 유사도 히트맵" text="대각선은 자기 자신과의 비교라 흰색으로 표시하고, 나머지는 라벤더(낮은 유사도)에서 민트(높은 유사도)로 읽어요. 가장 비슷한 쌍은 은은한 민트 빛으로 깜박입니다." /><div className="heatmap-layout"><div className="heatmap" style={{ gridTemplateColumns: `repeat(${Math.max(data.matrix.length, 1)}, minmax(0, 1fr))` }}>{data.matrix.flatMap((row, i) => row.map((value, j) => { const diagonal = i === j; const isBest = bestPair ? ((i === bestPair.i && j === bestPair.j) || (i === bestPair.j && j === bestPair.i)) : false; const intensity = Math.max(0, Math.min(1, value)); const low = [174, 190, 226]; const high = [116, 216, 185]; const rgb = low.map((channel, index) => Math.round(channel + (high[index] - channel) * intensity)); return <div key={`${i}-${j}`} className={`${diagonal ? "heatmap-diagonal" : ""} ${isBest ? "heatmap-best" : ""}`} style={{ background: diagonal ? "#fff" : `rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`, color: "#16312c" }}><small>{i + 1} · {j + 1}</small><b>{value.toFixed(2)}</b></div>; }))}</div><div className="cosine-visuals"><figure><img src="/cosine-formula.png" alt="코사인 유사도 공식" /><figcaption>두 벡터의 내적과 크기로 유사도를 계산해요.</figcaption></figure><figure><img src="/cosine-angle.png" alt="두 벡터 사이의 각도 세타 그림" /><figcaption>벡터 사이의 각 θ가 작을수록 문장이 더 비슷해요.</figcaption></figure></div></div><div className="heatmap-legend"><span><i className="blue-swatch" /> 라벤더: 낮은 유사도</span><span><i className="red-swatch" /> 민트: 높은 유사도</span><span><i className="white-swatch" /> 자기 자신과의 비교</span></div><p className="readout">가장 비슷한 문장 쌍: <b>댓글 {(bestPair?.i ?? 0) + 1}</b>와 <b>댓글 {(bestPair?.j ?? 1) + 1}</b></p></div>}
