...
const initial = [
  "성심당 빵은 맛있다 맛있다. 망고시루 망고시루도 맛있다. 직원은 친절하다.",
  "성심당 빵은 맛있다 맛있다. 망고시루 망고시루가 맛있다. 직원은 친절하다.",
  "성심당 빵은 맛있다 맛있다. 순수롤 순수롤도 부드럽다. 가격은 저렴하다.",
  "성심당 빵은 맛있다 맛있다. 순수롤 순수롤이 부드럽다. 가격은 저렴하다.",
  "성심당 빵은 맛있다 맛있다. 포장이 깔끔하고 메뉴가 다양하다.",
];
...
export default function Home() {
  ...
  const idfRows = [...data.vocabulary].sort((a, b) => data.df[b] - data.df[a] || a.localeCompare(b, "ko"));
      ...
      <section className="lab" id="lab">
        ...
        <div className="workbench">
          ...
          <div className={`stage ${motionEnabled ? "motion-on" : "motion-off"}`}>
            ...
            {step === 9 && <div className="panel"><PanelTitle n="09" title="코사인 유사도 히트맵" text="대각선은 자기 자신과의 비교라 흰색으로 표시하고, 나머지는 파란색(낮은 유사도)에서 빨간색(높은 유사도)으로 읽어요. 가장 비슷한 쌍은 빨간색으로 깜박입니다." /><div className="heatmap" style={{ gridTemplateColumns: `repeat(${Math.max(data.matrix.length, 1)}, minmax(0, 1fr))` }}>{data.matrix.flatMap((row, i) => row.map((value, j) => { const diagonal = i === j; const isBest = bestPair ? ((i === bestPair.i && j === bestPair.j) || (i === bestPair.j && j === bestPair.i)) : false; const intensity = Math.max(0, Math.min(1, value)); const red = Math.round(40 + intensity * 190); const blue = Math.round(215 - intensity * 170); return <div key={`${i}-${j}`} className={`${diagonal ? "heatmap-diagonal" : ""} ${isBest ? "heatmap-best" : ""}`} style={{ background: diagonal ? "#fff" : `rgb(${red}, 92, ${blue})`, color: intensity > 0.62 && !diagonal ? "#fff" : "#14251a" }}><small>{i + 1} · {j + 1}</small><b>{value.toFixed(2)}</b></div>; }))}</div><div className="heatmap-legend"><span><i className="blue-swatch" /> 낮은 유사도</span><span><i className="red-swatch" /> 높은 유사도</span><span><i className="white-swatch" /> 자기 자신과의 비교</span></div><p className="readout">가장 비슷한 문장 쌍: <b>댓글 {(bestPair?.i ?? 0) + 1}</b>와 <b>댓글 {(bestPair?.j ?? 1) + 1}</b></p></div>}
            {step === 10 && <div className="panel"><PanelTitle n="10" title="댓글을 핵심 단어로 군집화해요" text="왼쪽의 댓글을 읽고, TF-IDF 점수가 높은 핵심 단어를 공유하는 댓글끼리 오른쪽 그룹으로 묶어 보았어요. 각 댓글의 화살표가 실제로 연결된 그룹을 가리키며, 깜박이는 화살표는 댓글이 해당 군집으로 연결되는 방향을 보여줘요." /><div className="cluster-board"><div className="comment-cards"><h4>1. 댓글 분석</h4>{comments.map((comment, index) => { const groupIndex = commentGroupIndexes[index]; const groupLabel = groupIndex >= 0 ? clusterGroups[groupIndex].label : "분류 대기"; return <div className="comment-card" key={index}><span>댓글 {index + 1}</span><span className={`cluster-arrow comment-arrow ${groupIndex >= 0 ? `cluster-arrow-${groupIndex}` : "cluster-arrow-unassigned"}`} aria-label={`댓글 ${index + 1} → ${groupLabel}`}>→</span><p>{comment || "입력된 댓글이 없습니다."}</p></div>; })}</div><div className="cluster-column"><h4>2. 댓글 분류</h4>{clusterGroups.length ? clusterGroups.map((group, groupIndex) => <div className={`cluster-card cluster-card-${groupIndex}`} key={group.label}><div className="cluster-heading"><b>{group.label}</b><span>{group.documentIndexes.length}개 댓글</span></div><p className="cluster-keywords">핵심 단어 {group.keywords.map((word) => <em key={word}>{word}</em>)}</p><div className="cluster-members">{group.documentIndexes.map((docIndex) => <span key={docIndex}>댓글 {docIndex + 1}</span>)}</div></div>) : <p className="empty-filter">분류할 단어가 아직 없어요. 댓글을 먼저 입력해 주세요.</p>}</div></div></div>}
