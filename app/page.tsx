...
const initial = [
  "성심당 망고시루 진짜 최고예요! 망고시루 살 때 직원분도 너무 친절하셔서 감동했어요. 다음에도 망고시루 사러 친절한 성심당 또 갈게요.",
...
export default function Home() {
  ...
  const idfRows = [...data.vocabulary].sort((a, b) => data.df[b] - data.df[a] || a.localeCompare(b, "ko")).slice(0, 20);
      ...
      <section className="lab" id="lab">
        ...
        <div className="workbench">
          ...
          <div className={`stage ${motionEnabled ? "motion-on" : "motion-off"}`}>
            ...
            {(pdfMode || step === 9) && <div className="panel"><PanelTitle n="09" title="코사인 유사도 히트맵" text="대각선은 자기 자신과의 비교라 흰색으로 표시하고, 나머지는 라벤더(낮은 유사도)에서 민트(높은 유사도)로 읽어요. 가장 비슷한 쌍은 은은한 민트 빛으로 깜박입니다." /><div className="heatmap" style={{ gridTemplateColumns: `repeat(${Math.max(data.matrix.length, 1)}, minmax(0, 1fr))` }}>{data.matrix.flatMap((row, i) => row.map((value, j) => { const diagonal = i === j; const isBest = bestPair ? ((i === bestPair.i && j === bestPair.j) || (i === bestPair.j && j === bestPair.i)) : false; const intensity = Math.max(0, Math.min(1, value)); const low = [174, 190, 226]; const high = [116, 216, 185]; const rgb = low.map((channel, index) => Math.round(channel + (high[index] - channel) * intensity)); return <div key={`${i}-${j}`} className={`${diagonal ? "heatmap-diagonal" : ""} ${isBest ? "heatmap-best" : ""}`} style={{ background: diagonal ? "#fff" : `rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`, color: "#16312c" }}><small>{i + 1} · {j + 1}</small><b>{value.toFixed(2)}</b></div>; }))}</div><div className="heatmap-legend"><span><i className="blue-swatch" /> 라벤더: 낮은 유사도</span><span><i className="red-swatch" /> 민트: 높은 유사도</span><span><i className="white-swatch" /> 자기 자신과의 비교</span></div><p className="readout">가장 비슷한 문장 쌍: <b>댓글 {(bestPair?.i ?? 0) + 1}</b>와 <b>댓글 {(bestPair?.j ?? 1) + 1}</b></p></div>}
            {(pdfMode || step === 10) && <div className="panel"><PanelTitle n="10" title="댓글을 핵심 단어로 군집화해요" text="왼쪽의 댓글을 읽고, TF-IDF 점수가 높은 핵심 단어를 공유하는 댓글끼리 오른쪽 그룹으로 묶어 보았어요. 같은 색으로 표시하고, 가장 많은 댓글이 포함된 그룹만 반짝여요." /><div className="cluster-board"><div className="comment-cards"><h4>1. 댓글 분석</h4>{comments.map((comment, index) => { const groupIndex = commentGroupIndexes[index]; return <div className={`comment-card ${groupIndex >= 0 ? `cluster-comment-${groupIndex}` : "cluster-comment-unassigned"} ${groupIndex === largestClusterIndex ? "largest-cluster" : ""}`} key={index}><span>댓글 {index + 1}</span><p>{comment || "입력된 댓글이 없습니다."}</p></div>; })}</div><div className="cluster-column"><h4>2. 댓글 분류</h4>{clusterGroups.length ? clusterGroups.map((group, groupIndex) => <div className={`cluster-card cluster-card-${groupIndex}`} key={group.label}><div className="cluster-heading"><b>{group.label}</b><span>{group.documentIndexes.length}개 댓글</span></div><p className="cluster-keywords">핵심 단어 {group.keywords.map((word) => <em key={word}>{word}</em>)}</p><div className="cluster-members">{group.documentIndexes.map((docIndex) => <span key={docIndex}>댓글 {docIndex + 1}</span>)}</div></div>) : <p className="empty-filter">분류할 단어가 아직 없어요. 댓글을 먼저 입력해 주세요.</p>}</div></div></div>}
