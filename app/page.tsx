      ...
      <section className="lab" id="lab">
        ...
        <div className="workbench">
          ...
          <div className="stage">
            ...
            {step === 4 && <div className="panel"><PanelTitle n="04" title="TF: 단어 빈도를 세어요" text="TF(Term Frequency)는 한 댓글 안에서 단어가 몇 번 반복되었는지 보여줘요. 같은 댓글에서 2번 이상 반복된 단어는 주황색으로 강조했어요." />{data.cleaned.map((doc, i) => <div className="bar-row" key={i}><b>댓글 {i + 1}</b><div>{[...new Set(doc)].map((word) => { const count = doc.filter((w) => w === word).length; const isRepeated = count >= 2; return <span className={`bar ${isRepeated ? "repeated" : "single"}`} key={word} title={`${word}: 이 댓글에서 ${count}번 반복${isRepeated ? "된 단어" : "된 단어"}`} style={{ width: `${Math.max(64, count * 72)}px` }}>{word} <small>×{count}</small></span>; })}</div></div>)}</div>}
            ...
            {step === 6 && <div className="panel"><PanelTitle n="06" title="TF-IDF: 의미의 좌표 만들기" text="자주 나오면서도 다른 댓글에는 드문 단어에 더 높은 점수를 줘요. 주황색은 2개 이상 댓글에 공통으로 나온 단어, 초록색은 한 댓글에만 나온 단어예요." /><div className="filter-row" role="group" aria-label="TF-IDF 차트 보기 옵션"><button className={tfidfFilter === "all" ? "selected" : ""} onClick={() => setTfidfFilter("all")}>전체 단어 보기</button><button className={tfidfFilter === "common" ? "selected" : ""} onClick={() => setTfidfFilter("common")}>공통 단어만 보기</button><button className={tfidfFilter === "top5" ? "selected" : ""} onClick={() => setTfidfFilter("top5")}>상위 5개</button><button className={tfidfFilter === "top8" ? "selected" : ""} onClick={() => setTfidfFilter("top8")}>상위 8개</button></div><div className="score-legend"><span><i className="common-swatch" /> 주황색: 공통 단어</span><span><i className="single-swatch" /> 초록색: 한 댓글에만 나온 단어</span></div><p className="chart-note">공통 단어 수: <b>{data.tfidfRows.filter((item) => item.documentCount >= 2).length}개</b> · 현재 <b>{visibleTfidfRows.length}개</b> 표시 중</p><div className="score-grid">{visibleTfidfRows.map((item) => { const explanation = item.documentCount >= 2 ? `${item.word} (TF-IDF: ${item.score.toFixed(2)}) → 댓글 ${item.documentCount}개에 공통으로 등장해 여러 댓글을 연결하는 단어예요.` : `${item.word} (TF-IDF: ${item.score.toFixed(2)}) → 한 댓글에만 등장해 그 댓글의 특징을 보여주는 단어예요.`; return <div className={`score-item ${item.documentCount >= 2 ? "common" : "single"}`} key={item.word} title={explanation}><span>{item.word}</span><b>{item.score.toFixed(2)}</b><i style={{ height: `${35 + item.score * 34}px` }}/><em className="score-tooltip">{explanation}</em></div>; })}</div>{visibleTfidfRows.length === 0 && <p className="empty-filter">공통으로 등장한 단어가 아직 없어요. 전체 단어 보기를 선택해 보세요.</p>}<p className="chart-help">💡 TF-IDF 막대의 색을 함께 읽어 보세요. <b>주황색</b>은 여러 댓글에 공통으로 등장한 단어, <b>초록색</b>은 한 댓글에만 등장해 그 댓글의 특징을 보여주는 단어예요.</p></div>}
